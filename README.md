...
•  Go to Settings > Sources and add or edit an M3U source for the mosaic stream:
	•  Name: Multi4x (or any name, e.g., for ch199).
	•  URL: udp://127.0.0.1:4444
	•  Save and refresh the guide to ensure ch199 is available.
...
bash docker run --rm -it --network host \
...
Limitations
•  Host Mode Only: Currently configured for --network host. Bridge mode may require additional network configuration.
...
Future Improvements
•  Support bridge mode for multi-device streaming.  I would live to figure out how to run this in bridge mode but the stream output breaks everytime I try.  Can you help?
